...
export default function Contact() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // 1. Envoyer l'email à vous-même (notification)
      await emailjs.sendForm(
        "service_qsww15m",   // Service ID
        "template_apkc3b2",  // Template ID pour vous
        formRef.current
      )

      // 2. Envoyer l'auto-réponse au visiteur
      await emailjs.sendForm(
        "service_qsww15m",        // Même Service ID
        "template_09c959s",  // ID du template d'auto-réponse (à créer)
        formRef.current
      )
